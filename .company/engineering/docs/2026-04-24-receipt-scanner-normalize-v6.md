...
## 10. v6.2 追補（空白なし末尾「○○店」ヒューリスティック除去）

- 日付: 2026-04-24
- 起因: v6.1 完了後のオーナー追加要望
- 対象: `api/process.js` の `stripBranchSuffix()` 関数の「空白なし」分岐
- 影響範囲: `STORE_NORMALIZATION_RULES` 非マッチ時 かつ 空白を含まない表記の末尾「店」処理

### 10.1 方針

採用方針: **案 B + A ハイブリッド**

- **案B**: 空白なしケースで末尾の「漢字/カナ連鎖 + 店」を可変長除去
- **案A**: ルール登録は必要なものだけ残す（A-PRICE のようなアルファベット/記号含むチェーン用）

これにより、`STORE_NORMALIZATION_RULES` に明示登録していないチェーンについても、空白なし表記の末尾「○○店」を汎用的に剥がせる。

意図する変化:
- `イオンフードスタイル東心斎橋店` → `イオンフードスタイル`（新規対応）
- `業務スーパー高津店` → `業務スーパー`（ルール経由でもヒューリスティックでも可）
- `リカーマウンテン畳屋町店` → `リカーマウンテン`（ルール経由）

### 10.2 変更内容

#### 10.2.1 `stripBranchSuffix()` の空白なし分岐の改修
...
**After（v6.2）**:
```js
// 末尾「店」+ 直前の漢字/カナ連鎖（2〜8文字）を可変長除去
// 商店/支店/本店は保護済み（上流で return 済）
const heuristicRe = /[一-龯ぁ-んァ-ヶー々〇]{2,8}店$/u;
if (heuristicRe.test(s)) {
  return s.replace(heuristicRe, '').replace(/[・\-\s\u3000]+$/, '').trim();
}
// マッチしなければ末尾「店」1文字のみ除去（従来挙動）
return s.replace(/(?<!商)店$/, '').replace(/[・\-\s\u3000]+$/, '').trim();
```
...
2〜8文字の可変長にすることで、地名や町名の長さをある程度カバーしつつ、過剰除去のリスクを限定する。

#### 10.2.2 保護ガードの拡張

v6.1 までの上流ガード（`商店$` / `支店$`）に加え、**本店ガード**を追加する。

```js
if (/商店$/.test(s)) return s;
if (/支店$/.test(s)) return s;
if (/本店$/.test(s)) return s; // v6.2 新規
```

意図: 「〇〇屋本店」「〇〇本店」などは本店という概念を保持したい。本店/支店の区別は集計上の意味を持つため、ヒューリスティックに食わせない。

#### 10.2.3 `STORE_NORMALIZATION_RULES` の扱い

Tech Lead 判断: **v6 で追加した `業務スーパー` / `久世福商店` ルールは残す**。

根拠:
- 「業務スーパー銀座店」のような非漢字混在の支店名に対しても安定して `業務スーパー` に正規化できる。
- パターン明示性（意図が設計書を読まなくても分かる）。
- ヒューリスティックと二重にかかっても最終結果は同じで害なし（ルールマッチが先行）。

将来の整理候補ではあるが、v6.2 では削除しない。

### 10.3 期待出力（テスト）

| # | 入力 | 期待出力 | 根拠 |
|---|---|---|---|
| 1 | `イオンフードスタイル東心斎橋店` | `イオンフードスタイル` | 空白なし・ヒューリスティック（新規） |
| 2 | `業務スーパー高津店` | `業務スーパー` | ルール経由（優先）／ヒューリスティックでも可 |
| 3 | `セブンイレブン西心斎橋店` | `セブンイレブン` | ルール経由（v6 既存） |
| 4 | `リカーマウンテン畳屋町店` | `リカーマウンテン` | ルール経由（v6 既存） |
| 5 | `久世福商店` | `久世福商店` | `商店$` ガード |
| 6 | `ローソン銀行渋谷支店` | `ローソン銀行渋谷支店` | `支店$` ガード（v6.1） |
| 7 | `〇〇本店` | `〇〇本店` | `本店$` ガード（**v6.2 新規**） |
| 8 | `商店` | `商店` | `商店$` ガード |
| 9 | `店` | `店` | 1文字ガード |
| 10 | `A-PRICE 熱波千日前店` | `A-PRICE` | 空白あり・最終トークン破棄（v6.1） |
| 11 | `久世福商店 なんばウォーク店` | `久世福商店` | ルール経由（v6 既存、`商店` 保護） |
| 12 | `LAWSON 道頓堀` | `ローソン 道頓堀` | ルール経由（v6 既存、「店」で終わらない） |
| 13 | `ダイソー` | `ダイソー` | 「店」で終わらない |
| 14 | `エディオン` | `エディオン` | 「店」で終わらない |
| 15 | `〇〇商会` | `〇〇商会` | 「店」で終わらない |
| 16 | `KOBE SPICE` | `KOBE SPICE` | 「店」で終わらない |

### 10.4 リスクと緩和

| リスク | 想定ケース | 緩和策 |
|---|---|---|
| 「○○屋本店」系の過剰除去 | `〇〇屋本店` | `本店$` ガードで保護（10.2.2） |
| 数字混じり支店（`〇〇1号店`） | `業務スーパー1号店` | 数字は Unicode クラス範囲外のためヒューリスティックで未ヒット→末尾「店」1文字のみ除去、`業務スーパー1号` が残る。**許容**（想定頻度低） |
| 新興チェーンの誤破壊 | 未登録チェーン名が漢字/カナ 2〜8 文字＋「店」で終わる名称 | dry-run 必須。問題があれば `STORE_NORMALIZATION_RULES` に個別保護／明示ルール追加でフォールバック |
| 正規表現リテラル範囲 | `一-龯` の網羅性 | JIS 第2水準までカバー。特殊文字が必要になれば後続 patch で拡張 |
...
- **Read ツール禁止**（malware 誤判定回避）。ファイル参照は `grep` / `sed -n` などの Bash 経由、または GLM 呼び出しでコンテキスト取得する。
- 変更対象は `api/process.js` の `stripBranchSuffix` 関数 **のみ**。他の関数・ルールテーブルは触らない。
- 関数の **export 形式・引数シグネチャ・関数名** は維持する。

#### 手順
1. `grep -n "function stripBranchSuffix" api/process.js` で関数位置を特定。
2. `grep -n -A 30 "function stripBranchSuffix" api/process.js` で現状（v6.1）実装を Bash 経由で取得。
3. GLM に「10.2.1（空白なし分岐）」「10.2.2（本店ガード追加）」を反映した diff を生成させる。
4. Edit ツールで関数本体のみ置換。周辺行（呼び出し元・他関数）は変更しない。
5. 構文チェック: `node --check api/process.js` がパスすること。
6. テスト検証（Node ワンライナー）:
   ```
   node -e "const {stripBranchSuffix,normalizeStoreName}=require('./api/process.js'); \
   [['イオンフードスタイル東心斎橋店','イオンフードスタイル'], \
    ['業務スーパー高津店','業務スーパー'], \
    ['セブンイレブン西心斎橋店','セブンイレブン'], \
    ['リカーマウンテン畳屋町店','リカーマウンテン'], \
    ['久世福商店','久世福商店'], \
    ['ローソン銀行渋谷支店','ローソン銀行渋谷支店'], \
    ['〇〇本店','〇〇本店'], \
    ['商店','商店'], \
    ['店','店'], \
    ['A-PRICE 熱波千日前店','A-PRICE'], \
    ['久世福商店 なんばウォーク店','久世福商店'], \
    ['LAWSON 道頓堀','ローソン 道頓堀'], \
    ['ダイソー','ダイソー'], \
    ['エディオン','エディオン'], \
    ['〇〇商会','〇〇商会'], \
    ['KOBE SPICE','KOBE SPICE']].forEach(([i,e])=>{const a=normalizeStoreName(i); \
    console.log((a===e?'OK':'NG'), JSON.stringify(i),'→',JSON.stringify(a),'(期待:',JSON.stringify(e),')')});"
   ```
   - 全件 OK 出力をログとして Reviewer に提出。
7. `scripts/renormalize-stores.mjs` の dry-run を実行し、v6.1 完了時との差分が 10.3 の想定ケース（空白なし + 漢字/カナ末尾店）のみに収まることを確認。差分ログを提出。

#### 禁止事項
- 関数シグネチャ変更。
- `STORE_NORMALIZATION_RULES` テーブルの編集（v6.2 はヒューリスティック追加のみ）。
- `商店` / `支店` / `本店` 保護ガードの弱体化。
- v6 / v6.1 の確定挙動に対する退行。

### 10.6 Reviewer チェックリスト

- [ ] `stripBranchSuffix()` の空白なし分岐に 10.2.1 のヒューリスティック正規表現が追加されている
- [ ] 上流ガードに `本店$` が追加されている（10.2.2）
- [ ] 10.3 の期待出力 #1〜#16 が全て一致
- [ ] v6.1 の期待出力（9.2 の #1〜#7）が引き続き pass
- [ ] v6 の期待出力（4.1 の #1〜#18）が引き続き pass
- [ ] `商店` / `支店` / `本店` の 3 保護が全て機能
- [ ] 空白区切りあり表記の挙動が v6.1 から変わっていない
- [ ] `node --check api/process.js` 構文エラーなし
- [ ] `scripts/renormalize-stores.mjs` dry-run で想定外の破壊的差分が出ていない
- [ ] git diff が `stripBranchSuffix` 関数本体のみに収まっている

### 10.7 D チーム（統合）指示

- 単一関数改修のためコンフリクトなし。
- `scripts/renormalize-stores.mjs` dry-run ログを取得し、v6.1 本実装時の dry-run 結果と差分を比較。
- 差分が 10.3 の想定ケース（空白なし + 漢字/カナ 2〜8 文字 + 店）のみに収まっていることを確認。
- `本店` 系・`支店` 系・`商店` 系のレコードに差分が出ていないことを重点確認。
- 想定外の破壊的差分が 1 件でもあれば Tech Lead に即エスカレーション。

### 10.8 Tech Lead 最終承認観点

- git diff が `stripBranchSuffix` 関数本体 1 箇所のみ
- Reviewer チェックリスト全項目クリア
- dry-run 差分が 10.3 の想定範囲内
- v6 / v6.1 の承認済み挙動に退行がない
- `本店` 保護が新規に機能している（回帰テストで確認）

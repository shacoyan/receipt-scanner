...
- 業態テーブル（後述）は **category 判定の補助** であり、**store の補完に使ってはならない**。例えば「大阪宗右衛門町店」とだけ読み取れた場合、「リカーマウンテン大阪宗右衛門町店」のように既知チェーン名で補完することは **禁止**。支店名の断片のみなら \`store\` を null にする。
...
| ホームセンター | コーナン / カインズ / DCM / ナフコ / ビバホーム / アークランズ / ジョイフル本田 / ロイヤルホームセンター / ケーヨーD2 / ユニディ | 消耗品費 |
...
※ このテーブルは **category の判定にのみ使用**。上記チェーン名を store として出力して良いのは、**画像にそのチェーン名が明示されている場合に限る**。

...
**【誤判定実例・絶対に真似するな】（v3.2 追加）**:
以下の印字は「10% 単一税率の外税表示」であり、モードC で処理する:
  (外税10.0% 対象額 ¥623)
  10.0% 消費税等    ¥62
  現計             ¥685
この時、¥623 は本体額、¥62 は税額（本体額 × 0.1 相当）。両方を「10%対象 / 8%対象 の本体額」
と読み替えるのは絶対禁止。モードC で amount=685 単一分割、category=消耗品費
（ホームセンターは消耗品費）。

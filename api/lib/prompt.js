...
- amount: 合計金額を数値のみで出力（カンマ・円記号・ハイフン記号は全て除去）。
  ...
  - **【手書き通貨記号の誤認防止・v3.6 追加】** 日本の手書き領収証では、金額の先頭に通貨記号（¥ / ￥ / Y / \\）が付く慣習がある。手書きの「¥」は字形（縦棒 + 横線 2 本）が「4」と視覚的に酷似しており、Claude vision が誤認しやすい。以下の判定基準で「先頭の 4」が本当に数字か通貨記号かを慎重に判別すること:
    - **発動条件**: 正式領収書（宛名「○○ 様」「御中」を持つ手書き領収証）であり、金額欄の先頭が「4」と読み取れた場合。
    - **判定手順**:
      1. その「4」を金額の一部として読んだ場合の総額を計算する（例: 「4」+「8,700-」= 48,700）。
      2. その「4」を通貨記号「¥」と解釈した場合の金額を計算する（例: ¥ + 「8,700-」= 8,700）。
      3. 以下の **いずれか** に該当する場合、「4」は通貨記号「¥」と解釈し、本来の金額（後続部分）のみを採用する:
         - 「4」の **直後にカンマが無く**、続く数字列が 3〜4 桁でカンマ区切り（例: "4 8,700"「4」と「8,700」の間に微小な空白・点・カーブがある）。これは「¥」の横線 2 本目を「4」の右上ハネと誤認しているパターン。
         - 「4」を外した金額（例: 8,700 / 8,850 / 3,800）が、領収証フォーマット上「自然な手書き金額」として収まる桁数（3〜5 桁）であり、かつ「4」を含めた金額（48,700 / 48,850 / 43,800）が **3 万円を大きく超える** 場合。手書き領収証で 3 万円超は稀。
         - 金額欄の冒頭位置（罫線上の「金額」ラベル直後）に「4」が孤立して書かれており、後続数字との間に明確な間隔・カンマ・点がある場合。
    - **判定基準 (除外条件・over-correction 防止)**:
      - 印字レシート（サーマル印字、レジレシート、明細テープ）では本ルールを **発動しない**。印字フォントの「¥」と「4」は混同しない。
      - 「4」の直後に **カンマなしで数字が密着連続** しており、合算桁数が手書きとして自然（例: "41200" → 41,200 / "4500" → 4,500）な場合は、それは本物の「4 始まりの金額」である可能性が高く、本ルールを発動しない。
      - 「¥41,200-」のように、頭に明確に通貨記号らしき記号がある上で「41,200」と続く場合は、通貨記号と「4」は **別物** として両方を認識し、「4」を削除しない（=¥41,200 のまま）。
      - 手書きでも、書き手のクセで明らかに「4」を 4 として書いている（縦棒が短く、横線が斜め・右肩下がり等）ことが視覚的に明瞭な場合は、本ルールを発動しない。
    - **出力上の扱い**: 通貨記号と確定した「4」は数値から除外し、本来の金額のみを \`amount\` に出力する。判定に迷う場合は \`confidence: "medium"\` + \`uncertainty_reason: "手書き先頭の '4' が通貨記号 ¥ の可能性あり"\` を返し、人間確認に回す。
    - **具体例**:
